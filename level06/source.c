...
int auth(char *login, unsigned int serial){
	...
	size_t	len;
	if ((len = strnlen(login, 32)) <= 5)
		return 1;

	// will stop and handle the control to main initiation function if execve called
	if (ptrace(PTRACE_TRACEME, 0, 1, 0) == -1) //- why second arg is 1??? they are ignored after traceme
	{
		puts("\033[32m.---------------------------.");
	...
	int	serial_decoded;
	serial_decoded = ((0xff & login[3]) ^ 4919) + 6221293;
	...
	while (counter < len)
	{
		if (login[counter] <= 31)
			return 1;
		serial_decoded = serial_decoded + hash(login[counter]);
		counter++;
	}

	if (serial != serial_decoded)
		return 1;
...
/*
int auth(char *login, unsigned int serial){ - 0x8 is login 0x
	int	serial_decoded;
	serial_decoded = ((0xff & login[3]) ^ 4919) + 6221293;
   ...
   0x0804874e <+6>:	movl   $0x8048a63,0x4(%esp) -"/n" to 2nd arg
   0x08048756 <+14>:	mov    0x8(%ebp),%eax - login to 1st arg
   0x08048759 <+17>:	mov    %eax,(%esp)
   0x0804875c <+20>:	call   0x8048520 <strcspn@plt>
   0x08048761 <+25>:	add    0x8(%ebp),%eax login + strcspn result
   0x08048764 <+28>:	movb   $0x0,(%eax) put zero

	login[strcspn(login, "\n")] = 0;

   0x08048767 <+31>:	movl   $0x20,0x4(%esp) - 32 to 2nd arg
   0x0804876f <+39>:	mov    0x8(%ebp),%eax - ligin to first arg
   0x08048772 <+42>:	mov    %eax,(%esp)
   0x08048775 <+45>:	call   0x80485d0 <strnlen@plt>
   0x0804877a <+50>:	mov    %eax,-0xc(%ebp) move result to var at 12

   0x0804877d <+53>:	push   %eax - to stack
   0x0804877e <+54>:	xor    %eax,%eax nullify eax - if it is 0 jump to 61
   0x08048780 <+56>:	je     0x8048785 <auth+61>
   0x08048782 <+58>:	add    $0x4,%esp - add 4 to esp
   0x08048785 <+61>:	pop    %eax -if the result is not 0 we add space to stack - canary?

   0x08048786 <+62>:	cmpl   $0x5,-0xc(%ebp) -with 5
   ...
   0x08048796 <+78>:	movl   $0x0,0xc(%esp) to 4th
   0x0804879e <+86>:	movl   $0x1,0x8(%esp) 1 to 3rd
   0x080487a6 <+94>:	movl   $0x0,0x4(%esp) to 2nd
   0x080487ae <+102>:	movl   $0x0,(%esp) to first traceme
   0x080487b5 <+109>:	call   0x80485f0 <ptrace@plt>

   0x080487ba <+114>:	cmp    $0xffffffff,%eax with -1
   0x080487bd <+117>:	jne    0x80487ed <auth+165> if not equal skip next steps
 ...
 canary creation
   ...
   0x080488d4 <+91>:	mov    %eax,0x8(%esp) stdin to 3rd arg
   0x080488d8 <+95>:	movl   $0x20,0x4(%esp) - 32 to 2nd arg
   0x080488e0 <+103>:	lea    0x2c(%esp),%eax - var 44 on stack
   0x080488e4 <+107>:	mov    %eax,(%esp) var to 1st arg
   ...
   0x08048922 <+169>:	lea    0x28(%esp),%edx - var 40 on stack (44-40 int)
   0x08048926 <+173>:	mov    %edx,0x4(%esp) var to 2nd arg
   0x0804892a <+177>:	mov    %eax,(%esp) "%u" to 1st arg
   ...
   0x08048936 <+189>:	mov    %eax,0x4(%esp) - int var40 to 2d arg
   0x0804893a <+193>:	lea    0x2c(%esp),%eax
   0x0804893e <+197>:	mov    %eax,(%esp) var44 to 1st arg
   0x08048941 <+200>:	call   0x8048748 <auth>
   0x08048946 <+205>:	test   %eax,%eax compare with 0
   0x08048948 <+207>:	jne    0x8048969 <main+240> - sets return to 1
   ...
   0x08048967 <+238>:	jmp    0x804896e <main+245> - return 0
